  ...
  return createPortal(
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          "pointer-events-auto fixed inset-y-0 flex h-dvh w-[min(22rem,calc(100vw-1rem))] max-w-[88vw] flex-col overflow-hidden",
...
>(function AnimatedSidebarGroup({ className, ...props }, forwardedRef) {
  ...
  return (
    <div
      ...
      className={cn(
        "flex w-full min-w-0 flex-col py-1.5 transition-[padding] duration-200",
        collapsed ? "px-3.5" : "px-1",
        className,
      )}
    />
  );
...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <div
      ...
      className={cn(
        "overflow-hidden px-2 text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground transition-[height,margin,opacity,transform] duration-150",
        collapsed ? "mb-0 h-0 -translate-y-1 opacity-0" : "mb-1 h-7 opacity-100",
        className,
      )}
    >
      {children}
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      pillContainerClassName="inset-y-auto top-0 h-10"
...
export function AnimatedSidebarMenuButton({
  ...
  const interactiveClassName = cn(
    "relative flex min-h-10 w-full min-w-0 items-center gap-2.5 overflow-hidden rounded-xl text-left text-sm font-medium outline-none",
    "text-muted-foreground transition-[padding,color] duration-200 hover:text-foreground",
    "focus-visible:bg-muted/70 focus-visible:ring-2 focus-visible:ring-ring",
    panel.collapsed ? "px-2.5" : "px-4",
