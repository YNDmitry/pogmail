...
export const Route = createFileRoute("/_app/admin/domains")({
  component: Domains,
});
          ...
          <ul className="divide-y divide-seam">
            {domains.data.map((domain) => (
              <li key={domain.id} className="space-y-2 px-4 py-3">
                      ...
                      ask({
                        ...
                        onConfirm: () =>
                          remove.mutate(domain.id, {
                            ...
                            onError: (error) =>
                              toast.fail("Could not remove it", String(error)),
                          }),
                      })
                    }
                  >
                ...
                {inspecting === domain.id ? (
                  <DnsPanel domainId={domain.id} />
                ) : null}
              </li>
            ))}
          </ul>
        ...
        <tbody className="divide-y divide-seam">
          {dns.data?.records.map((record) => (
            <tr key={record.id}>
              ...
              <td className="machine px-3 py-1.5 break-all">
                {record.content}
              </td>
              <td className="px-3 py-1.5">
                {record.managedByPogmail ? (
                  <Tag tone="accent">Pogmail</Tag>
                ) : (
