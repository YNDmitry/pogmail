...
export const Route = createFileRoute("/_app/admin/webhooks")({ component: Webhooks });
...
function Webhooks() {
	...
	const create = useCreate<Record<string, unknown>, Webhook>(qk.webhooks, "/api/webhooks");
	...
	return (
		<div className="space-y-5">
			<header className="flex flex-wrap items-end justify-between gap-4">
				<div>
					...
					<p className="mt-1 max-w-prose text-sm text-ink-2">
						...
						<Machine>X-Postbox-Signature</Machine>. Failures retry with a growing delay, and an endpoint
						that fails twenty times running is switched off.
					</p>
				</div>
				<Button size="sm" onClick={() => setOpen(true)} disabled={!mailboxes.data?.length}>
						...
						{hooks.data.map((hook) => (
							<li key={hook.id} className="space-y-2 px-4 py-3">
								<div className="flex flex-wrap items-center gap-3">
									<Machine className="min-w-0 flex-1 truncate text-sm">{hook.url}</Machine>
									...
									<Button
										...
										onClick={async () => {
											try {
												...
												toast.fail("Test failed", error instanceof ApiError ? error.message : undefined);
											}
										}}
									>
									...
									<Button
										size="sm"
										variant="ghost"
										onClick={() => setInspecting(inspecting === hook.id ? null : hook.id)}
									>
										Deliveries
									</Button>
									...
									<Button
										...
										onClick={() =>
											remove.mutate(hook.id, { onSuccess: () => toast.ok("Webhook deleted") })
										}
									>
								...
								{inspecting === hook.id ? <Deliveries webhookId={hook.id} /> : null}
							</li>
						))}
					</ul>
				</Card>
			) : (
				<Empty title="No webhooks" body="Point one at your service to get a POST whenever mail arrives." />
			)}
			...
			<Modal open={secret !== null} onClose={() => setSecret(null)} title="Copy the signing secret">
				<div className="space-y-4">
					<p className="text-sm text-ink-2">
						Verify it against the <Machine>X-Postbox-Signature</Machine> header, which is
...
function Deliveries({ webhookId }: { webhookId: string }) {
	const toast = useToast();
	const deliveries = useList<Delivery>(qk.deliveries(webhookId), `/api/webhooks/${webhookId}/deliveries`);
			...
			<table className="w-full text-left text-xs">
				...
				<tbody className="divide-y divide-seam">
					{deliveries.data.map((delivery) => (
						<tr key={delivery.id}>
							<td className="machine px-3 py-1.5">{fullDate(delivery.createdAt)}</td>
							...
							<td className="px-3 py-1.5">
								...
								{delivery.errorSnippet ? (
									<span className="ml-2 text-ink-3">{delivery.errorSnippet.slice(0, 60)}</span>
								) : null}
							</td>
