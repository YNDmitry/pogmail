...
function Calendar() {
    ...
    (next: { month?: Date; day?: Date; view?: CalendarView }) => {
      void navigate({
        search: (prev) => ({
          ...
          ...(next.day
            ? { day: dayParam(next.day), month: monthParam(next.day) }
            : {}),
  ...
  const events = useQuery({
    ...
    queryFn: async () =>
      (
        await api.get<{ items: CalendarEvent[] }>("/api/calendar/events", {
          query: { from, to },
        })
      ).items,
  });
  ...
  const step = useCallback(
    (direction: -1 | 1) => {
      if (view === "week") {
        show({
          day: addDays(week[0] ?? startOfDay(new Date()), direction * 7),
        });
        return;
      }
      show({
        month: new Date(cursor.getFullYear(), cursor.getMonth() + direction, 1),
      });
    },
  ...
  const monthEvents = (events.data ?? [])
    .filter(
      (event) => new Date(event.startsAt).getMonth() === cursor.getMonth(),
    )
    .toSorted(
  ...
  return (
    <div className="mx-auto space-y-5 px-6 py-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          <input
            ...
            tabIndex={-1}
            onChange={(event) =>
              void importIcs(event.target.files?.[0] ?? null)
            }
          />
          <Button
            size="sm"
          ...
          <Button
            ...
            size="sm"
            variant="ghost"
            title="Download this range as .ics"
          >
      ...
      {view === "week" ? (
        <Card className="overflow-hidden">
          <WeekGrid
            ...
            events={events.data ?? []}
            onOpen={(event) => {
              const full = (events.data ?? []).find(
                (entry) => entry.id === event.id,
              );
              if (full) setDraft(draftFromEvent(full));
            }}
            onCreate={(start, end) =>
              setDraft({ ...draftForDay(start), startsAt: start, endsAt: end })
            }
                    ...
                    {dayEvents.slice(0, 3).map((event) => (
                      <li key={event.id}>
                        <EventChip
                          ...
                          onPickUp={() =>
                            setDragging({ event, from: startOfDay(day) })
                          }
                        />
                      </li>
                    ))}
      ...
      >
        <ul className="divide-y divide-border">
          {(openDay
            ? (byDay.get(startOfDay(openDay).getTime()) ?? [])
            : []
          ).map((event) => (
            <li key={event.id}>
              <AgendaRow
      ...
      >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field
            label="Ends"
            error={
              invalid ? "An event cannot end before it starts." : undefined
            }
          >
            <DateTimePicker
              label="Ends"
        ...
        {draft.id && parseAttendees(attendees).length > 0 ? (
          <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border px-3 py-2">
            ...
            >
              ...
              {inviting
                ? "Sending…"
                : `Invite ${parseAttendees(attendees).length}`}
            </Button>
