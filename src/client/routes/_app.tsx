...
const COMPACT_KEY = "pogmail-shell-compact";
...
function AppLayout() {
  ...
  const mailboxId = useRouterState({
    select: (state) =>
      (state.location.search as { mailboxId?: string }).mailboxId ?? null,
  });
    ...
    () => [
      {
        ...
        items: MAIL_VIEWS.map((view) => ({
          ...
          href: mailboxId
            ? `/mail/${view.slug}?mailboxId=${mailboxId}`
            : `/mail/${view.slug}`,
          badge:
            view.slug === "starred"
  ...
  const pageTitle = useMemo(() => {
    ...
    if (section === "settings")
      return SETTINGS_TITLES[rest ?? ""] ?? "Settings";
    if (section === "admin")
