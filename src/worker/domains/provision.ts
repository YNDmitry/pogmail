...
import { createDnsRecord, deleteDnsRecord, findZoneByHostname, listDnsRecords } from "../cloudflare/zones";
...
export async function provisionDomain(
	...
	const domain = await db.select().from(domains).where(eq(domains.id, domainId)).get();
	if (!domain) throw new Error(`Domain ${domainId} disappeared mid-provision`);

	const cf = CloudflareClient.fromEnv(env);

	try {
		const settings = await getRoutingSettings(cf, domain.zoneId).catch(() => null);
 ...
 */
export async function deprovisionDomain(env: Env, db: Database, domainId: string): Promise<void> {
	const domain = await db.select().from(domains).where(eq(domains.id, domainId)).get();
	if (!domain) return;
	...
	for (const rule of rules) {
		if (!rule.name?.startsWith("postbox:")) continue;
		await deleteRoutingRule(cf, domain.zoneId, rule.tag).catch(() => undefined);
	}

	const records = await db.select().from(domainRecords).where(eq(domainRecords.domainId, domain.id)).all();
	for (const record of records) {
		await deleteDnsRecord(cf, domain.zoneId, record.cloudflareRecordId).catch(() => undefined);
	}

	// Only turn routing off if we were the ones who turned it on.
	if (domain.routingEnabled) await disableRouting(cf, domain.zoneId).catch(() => undefined);
}
...
export async function provisionMailboxRule(
	...
	const cf = CloudflareClient.fromEnv(env);
	const rule = await createWorkerRule(cf, zoneId, address, env.CF_EMAIL_WORKER_NAME);
	return rule.tag ?? null;
}

export async function removeMailboxRule(env: Env, zoneId: string, tag: string): Promise<void> {
	const cf = CloudflareClient.fromEnv(env);
