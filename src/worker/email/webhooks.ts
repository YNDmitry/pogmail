...
export async function runDelivery(
  ...
  try {
    const response = await fetch(target.url, {
      ...
      headers: {
        ...
        "x-pogmail-event": delivery.event,
        "x-pogmail-delivery": delivery.id,
        "x-pogmail-signature": await sign(target.secret, body),
      },
      body,
  ...
  const retry: WebhookRetryMessage = {
    kind: "webhook-retry",
    deliveryId: delivery.id,
    attempt: nextAttempt,
  };
  await env.OUTBOUND_QUEUE.send(retry, {
