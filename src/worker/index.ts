...
import { backupSettings, backups, mailboxes, messages, users } from "@/db/schema";
import { api } from "./api";
...
import { isInbound, isOutbound, isWebhookRetry, type InboundQueueMessage, type QueuePayload } from "./email/types";
...
const FORWARDED_HEADER = "x-postbox-forwarded";
...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (url.pathname === "/api/realtime") {
			const user = await resolveSession(getDb(env.DB), readSessionCookie(request));
	...
	async email(message, env): Promise<void> {
		...
		if (decision.action === "forward") {
			...
			await message.forward(decision.to, new Headers({ [FORWARDED_HEADER]: "1" }));
			return;
		}
		...
		if (owner?.forwardingEmail) {
			await message
				.forward(owner.forwardingEmail, new Headers({ [FORWARDED_HEADER]: "1" }))
				.catch((error: unknown) => console.error("account forward failed", error));
		}
	},
	...
	async queue(batch, env): Promise<void> {
		for (const item of batch.messages) {
			...
			try {
				...
				else if (isWebhookRetry(payload)) await runDelivery(env, payload.deliveryId, payload.attempt);
	...
	async scheduled(_controller, env): Promise<void> {
		...
		await db
			...
			.where(and(isNotNull(messages.snoozedUntil), lte(messages.snoozedUntil, new Date())));
		...
		await env.BACKUP_WORKFLOW.create({ id: row.id, params: { backupId: row.id } });
		await db.update(backupSettings).set({ lastRunAt: new Date() }).where(eq(backupSettings.id, settings.id));
	},
...
function isBackupDue(settings: typeof backupSettings.$inferSelect): boolean {
	...
	if (settings.scheduleType === "weekly" && now.getUTCDay() !== (settings.scheduleValue ?? 0)) return false;
	if (settings.scheduleType === "monthly" && now.getUTCDate() !== (settings.scheduleValue ?? 1)) return false;
